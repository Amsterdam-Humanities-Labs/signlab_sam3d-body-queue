...
- `lockFile.php` claims a recording by writing `<take>.s3b_lock`. It refuses if a lock or result already exists.
- `upload.php` stores `<take>.sam3dbody` in the same folder. `list_uploads.php` and `get_sam3dbody_files.php` return JSON listings.
